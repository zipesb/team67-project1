/*
* CODE IN THIS FILE WAS ALMOST ENTIRELY TAKEN FROM THE FOLLOWING REPO: https://github.com/ThomasFoydel/MERN-image-upload
* THIS CODE IS FROM THE FOLLOWING YOUTUBE TUTORIAL: https://www.youtube.com/watch?v=OvbRLY1QRIk&ab_channel=SingleWingAcademy
*/
...
function checkFileType(file, cb) {
  ...
  const filetypes = /jpeg|jpg|txt|pptx|csv|png|pdf|gif/;
...
const uploadMiddleware = (req, res, next) => {
  const upload = store.single('resource');
  upload(req, res, function (err) {
    if (err instanceof multer.MulterError) {
      return res.status(400).send('File too large');
    } else if (err) {
      // check if our filetype error occurred
      if (err === 'filetype') return res.status(400).send('Filetype not allowed');
...
router.post('/upload/', uploadMiddleware, async (req, res) => {
  ...
  const resourceEntry = { id: file.id, filename: file.originalname };
  ClassModel.findByIdAndUpdate(classID,
    { "$push": { "resources": resourceEntry } },
    function (err, result) {
      if (err) {
        console.log("Could not update class page!")
        deleteResource(id);
        return res.status(400).send('Could not update class page!');
      } 
    }
  );
...
const deleteResource = (id) => {
  if (!id || id === 'undefined') return res.status(400).send('no resource id');
  const _id = new mongoose.Types.ObjectId(id);
  gfs.delete(_id, (err) => {
    if (err) return res.status(500).send('resource deletion error');
  });
};
